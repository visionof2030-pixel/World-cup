...
<script>
  ...
  // ======================= ترجمة الأسماء (محسنة بشكل كبير) =========================
  // خريطة شاملة للترجمة من الفارسية/الإنجليزية إلى العربية
  const nameMapping = new Map([
    ...
    // البرتغال (تمت إضافة مرادفات فارسية إضافية)
    ["پرتغال", "البرتغال"], ["پرتقال", "البرتغال"], ["Portugal", "البرتغال"], ["پرتغال", "البرتغال"],
    // الكونغو الديمقراطية (مرادفات متعددة)
  ...
  // دالة لتطبيع النص (إزالة التشكيل، توحيد الألف والياء والكاف، إلخ)
  function normalizeName(str) {
    if (!str) return "";
    // إزالة التشكيل (حركات)
    str = str.normalize("NFD").replace(/[\u064B-\u065F]/g, "");
    // استبدال الألف المقصورة بألف عادية
    str = str.replace(/[ى]/g, "ا");
    // إزالة الهمزات
    str = str.replace(/[أإآ]/g, "ا");
    // تحويل التاء المربوطة إلى هاء
    str = str.replace(/ة/g, "ه");
    // تحويل الكاف الفارسية (ک) إلى كاف عربية
    str = str.replace(/[ک]/g, "ك");
    // تحويل الياء الفارسية (ی) إلى ياء عربية
  ...
  // دالة للبحث عن تطابق جزئي (إذا لم يوجد تطابق تام)
  function partialMatch(raw, normalized) {
    // نبحث في الخريطة عن مفتاح يحتوي على النص المطبع أو العكس
    for (let [key, value] of nameMapping) {
      let keyNorm = normalizeName(key);
      // إذا كان النص المطبع يحتوي على المفتاح المطبع أو العكس
  ...
  // دالة الترجمة الرئيسية (محسنة جداً)
  function translateToArabic(raw) {
    ...
    // 3. البحث غير حساس لحالة الأحرف (للإنجليزية)
    ...
    // 4. البحث الجزئي (مفيد للأسماء المركبة أو التي بها كلمات إضافية)
    let partial = partialMatch(trimmed, normalized);
    if (partial) {
      return partial;
    }
    // 5. إذا لم يتم العثور، نعيد النص الأصلي مع تحذير
  ...
  async function loadPreviousGames() {
    ...
    try {
      ...
      previousGamesData = finished.map(game => {
        ...
        console.log(`🔍 قراءة مباراة: home="${homeRaw}", away="${awayRaw}"`);
      ...
      console.log(`📊 تم تحميل ${previousGamesData.length} مباراة سابقة.`);
  ...
  function calculateStandings() {
    ...
    previousGamesData.forEach(game => {
      ...
      if (!groupName) {
        console.warn(`⚠️ المباراة بين ${homeAr} و ${awayAr} لا تنتمي لأي مجموعة معروفة.`);
        return;
      }
